...
def test_leq(lhs, rhs, abs_tol, err_str, do_print: bool):
    if do_print:
        print(f"{lhs} <= {rhs}", err_str)
    assert lhs <= rhs + abs_tol, err_str


def test_eq(lhs, rhs, abs_tol, err_str, do_print: bool):
    if do_print:
        print(f"{lhs} == {rhs}", err_str)
    assert math.isclose(lhs, rhs, rel_tol=0.0, abs_tol=abs_tol), err_str


def test_validity(params: ParamsFCCA, result: ResultFCCA, do_print: bool):
    ...
    test_obj(result, params.abs_tol, do_print)
    test_constraints(params, result, do_print)


def test_obj(result: ResultFCCA, abs_tol: float, do_print: bool):
    ...
    test_eq(result.solver_obj, result.cost_total, abs_tol, _str, do_print)


def test_constraints(params: ParamsFCCA, result: ResultFCCA, do_print: bool):
    ...
    for i in veh_type_list:
        for j in outer_ring_id_list:
            ...
            rhs = n[i][j]
            test_leq(lhs, rhs, params.abs_tol, constr_n, do_print)
    if do_print:
    ...
    for j in params.ring_id_list:
        ...
        rhs = 1
        test_eq(lhs, rhs, params.abs_tol, constr_n, do_print)
    if do_print:
    ...
    for i in veh_type_list:
        for j in params.ring_id_list:
            ...
            test_leq(lhs, rhs, params.abs_tol, f"bigM_n_{i}_{j}", do_print)
            lhs = y[i][j]
            rhs = math.ceil(big_M_radius * x[i][j])
            test_leq(lhs, rhs, params.abs_tol, f"bigM_y_{i}_{j}", do_print)
            lhs = l[i][j]
            rhs = math.ceil(big_M_radius * x[i][j])
            test_leq(lhs, rhs, params.abs_tol, f"bigM_l_{i}_{j}", do_print)
    if do_print:
    ...
    for i in params.crowd_veh_types:
        ...
        test_leq(lhs, z_s, params.abs_tol, constr_n, do_print)
    if do_print:
